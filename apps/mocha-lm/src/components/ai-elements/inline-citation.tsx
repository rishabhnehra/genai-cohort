...
function sourceTriggerLabel(sources: string[]) {
  ...
  try {
    ...
    return (
      <>
        {hostname} {sources.length > 1 ? `+${sources.length - 1}` : null}
      </>
    );
  } catch {
    return (
      <>
        {first} {sources.length > 1 ? `+${sources.length - 1}` : null}
      </>
    );
  }
}
...
export const InlineCitationCardTrigger = ({
  ...
  <HoverCardTrigger
    render={
      <Badge
        className={cn("ml-1 cursor-pointer rounded-full align-middle", className)}
        variant="secondary"
        {...props}
      />
    }
  >
    {children ?? sourceTriggerLabel(sources)}
  </HoverCardTrigger>
);
